...
async def main():
    #Example usage
    ingestor = BatchIngester()

    #Get recent AI papers about LLM
    query = """cat:cs.AI AND submittedDate:[20240101 TO 20241231]"""
    processed = await ingestor.ingest_papers(query=query, max_papers=50,batch_size=5)

    logger.info(f"Processed {processed} papers")

    #Show error summary if any
    if ingestor.error_log.exists():
        with open(ingestor.error_log) as f:
            errors = [json.loads(line) for line in f]
            if errors:
                 logger.warning(f"Encountered {len(errors)} errors during ingestion")
                 for e in errors[:5]:
                     logger.warning(f"Paper {e['paper_id']} failed in {e['stage']}: {e['error']}")

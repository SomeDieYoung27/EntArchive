...
class PaperFetcher:
    ...
    async def fetch_papers(self,query:Optional[str]=None,paper_ids: Optional[List[str]] = None,max_results:int = 10,offset:int = 10) ->List[Dict]:
        """
        ...
        try:
            ...
            if paper_ids:
                ...
                search = arxiv.Search(query=query or "cs.AI",max_results=max_results,offset=offset)
    ...
    async def fetch_paper_content(self,paper_id:str) -> dict:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                pdf_path = await self.download_pdf(paper_id)
                return {
